...
df %>% group_by(sex, discipline) %>% summarise(nsex = n())
...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(selectizeInput(inputId = "majorchoice",
                                ...
                                choices = factor(df$major1))),
    mainPanel(plotOutput(outputId = "majorplot"),
              tableOutput(outputId = "majortable"))
  )
)
...
server <- function(input, output, session) {
  ...
  df_major %>% 
  output$majortable <- renderDataTable(df, options = list())
}
